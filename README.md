...
## Играбельная сборка онлайн

Готовая WebGL-сборка (результат Шага 5) опубликована через GitHub Pages и доступна без локального сервера:

**<https://neckrite.github.io/2DPlatformerLab/>**

Файлы сборки лежат в папке `docs/` этого репозитория (первая страница открывается автоматически; после полной загрузки видна сцена проекта).
